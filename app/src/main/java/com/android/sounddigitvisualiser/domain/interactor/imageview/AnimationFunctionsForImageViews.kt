...
class AnimationFunctionsForImageViews : AnimationsForImages {
    ...
    override fun handleScale(list: List<View>, volume: Int, MAX_RECORD_AMPLITUDE: Int, scaleFactor: Double) {
    ...
    override fun handleRotation(array: Array<View>, volume: Int, reactionTimer: Long) {
        for (item in array) {
            rotationAnimationArray = rotationAnimationTypes.rotationAnimations(item,volume,reactionTimer)
...
//            val vol = (volume / 10000)
    ...
    override fun handleMovement(list: List<View>, volume: Int, reactionTimer: Long, fromX: Int, toX: Int, fromY: Int, toY: Int) {
                for (item in list){
